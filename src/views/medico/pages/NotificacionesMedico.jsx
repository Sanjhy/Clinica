...
export default function NotificacionesMedico({ user }) {
    const [contactos, setContactos] = useState([]);
    const [rolDestino, setRolDestino] = useState('ENFERMERA');
    ...
    useEffect(() => {
        const fetchUsuarios = async () => {
            try {
                ...
                let res;
                if (rolDestino === 'ENFERMERA') {
                    res = await axios.get('http://localhost:8080/api/usuarios/enfermeras', config);
            ...
            <Box sx={{ width: 320, display: 'flex', flexDirection: 'column', borderRight: '1px solid', borderColor: 'divider', bgcolor: '#f8fafc' }}>
                <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider', bgcolor: '#fff' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5, mb: 2 }}>
                        <Button 
                            variant={rolDestino === 'ENFERMERA' ? 'contained' : 'outlined'} 
                            fullWidth 
                            size="small"
                            onClick={() => setRolDestino('ENFERMERA')}
                ...
                <List sx={{ flexGrow: 1, overflowY: 'auto', p: 0 }}>
                    ...
                    {/* Opción de Grupo Interno (MEDICOS) */}
                    <ListItem 
                        button 
                        onClick={() => setChatActivo({ tipo: 'GRUPO', id: 'MEDICO', nombre: 'General Médicos', avatar: '👨‍⚕️' })}
                        sx={{ 
                            borderBottom: '1px solid', borderColor: 'divider',
                            bgcolor: chatActivo?.id === 'MEDICO' ? 'rgba(59, 130, 246, 0.08)' : 'transparent',
                            borderLeft: chatActivo?.id === 'MEDICO' ? '4px solid #3b82f6' : '4px solid transparent',
                            '&:hover': { bgcolor: 'rgba(59, 130, 246, 0.04)' }
                        }}
                    >
                        <ListItemAvatar>
                            <Avatar sx={{ bgcolor: chatActivo?.id === 'MEDICO' ? '#3b82f6' : 'grey.400' }}>👨‍⚕️</Avatar>
                        </ListItemAvatar>
                        <ListItemText 
                            primary="General Médicos" 
                            secondary="Chat del Departamento"
                            primaryTypographyProps={{ fontWeight: chatActivo?.id === 'MEDICO' ? 700 : 500, color: '#0f172a' }}
                        />
                    </ListItem>
